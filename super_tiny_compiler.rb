...
#  and Code Generation
#
#  1. *Parsingis taking raw code and turning it into a more abstract
#     representation of the code.
#
#  2. *Transformationtakes this abstract representation and manipulates to do
#     whatever the compiler wants it to.
#
#  3. *Code Generationtakes the transformed representation of the code and
...
def tokenizer(input)
  ...
  while current < input.length
    ...
    if char === '('
      ...
      # And we `next` onto the next cycle of the loop.
      next
    end
...
def parser(tokens)
  ...
  # def walk()
    ...
    if token[:type] == 'paren' && token[:value] == '('
      ...
      # We increment `current` *againto skip the name token.
      current += 1
      token = tokens[current]
      ...
      # So we create a `while` loop that will next until it encounters a
...
def traverser(ast, visitor)
  ...
  traverseNode = lambda do |node, parent|

    # A `traverseArray` function that will allow us to iterate over an array and
    # call the next function that we will define: `traverseNode`.
    traverseArray = lambda do |array, parent|
      array.each { |child| traverseNode.call(child, parent) }
    end

    # We start by testing for the existence of a method on the visitor with a
    # matching `type`.
    method = visitor[node[:type].to_sym]
    ...
    case node[:type]
      ...
      when 'Program'
        traverseArray.call(node[:body], node)
        break;

      # Next we do the same with `CallExpressions` and traverse their `params`.
      when 'CallExpression'
        traverseArray.call(node[:params], node)
        break;
...
def transformer(ast)
  ...
  #
  # Just take note that the context is a reference *fromthe old ast *tothe
  # new ast.
  ...
  traverser(ast, {
    ...
    NumberLiteral: lambda do |node, parent|
    ...
    CallExpression: lambda do |node, parent|
...
def codeGenerator(node)
  ...
  case node[:type]
    ...
    when 'Program'
      return node[:body].map {|el| codeGenerator(el)}.join('\n')

    # For `ExpressionStatements` we'll call the code generator on the nested
    # expression and we'll add a semicolon...
    when 'ExpressionStatement'
      return (codeGenerator(node[:expression]) + ';')
      # (semicolon...because we like to code the *correctway)
    ...
    when 'CallExpression'
      return (
        ...
        node[:arguments].map { |el| codeGenerator(el) }.join(', ') + ')'
      )
